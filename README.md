...
2. Each character determines one pin action (LOW, HIGH, or TOGGLE).
3. The `apply_action()` function executes the command using HAL functions.

   ```c
   void apply_action(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin, char action)
   {
       switch (action)
       {
           case '0':
           ...
           default:
               // Invalid command, optionally handle error
               break;
       }
   }
...
* Suitable for entry-level embedded systems and communication experiments
...
| `uart_gpio_toggle.ioc`                | STM32CubeIDE configuration (pinout and clock setup)     |
| `Proteus/simulation_sch_STM32.pdsprj` | Proteus simulation file                                 |
| `Proteus/Result.png`                  | Screenshot of Proteus simulation results                |
